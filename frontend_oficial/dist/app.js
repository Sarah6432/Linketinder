...
const formLoginCandidato = document.getElementById('formLoginCandidato');
...
if (formVaga) {
    formVaga.addEventListener('submit', (e) => {
        ...
        const desc = document.getElementById('descVaga').value.trim();
        const skills = document.getElementById('skillsVaga').value.trim();
        if (!titulo || !skills) {
...
function renderizarTelaPerfil() {
    ...
    if (visao === 'candidato') {
        ...
        if (secaoCandidatos)
            secaoCandidatos.style.display = 'none';
        if (secaoVagas)
        ...
        if (secaoCandidatos)
            secaoCandidatos.style.display = 'block';
    }
    const candidatos = banco.listarCandidatos();
    const vagas = JSON.parse(localStorage.getItem('vagas') || '[]');
    ...
    if (visao === 'empresa')
        inicializarGrafico(candidatos);
}
function inicializarGrafico(candidatos) {
    const ctx = document.getElementById('graficoSkills');
    if (!ctx)
        return;
    const contagem = {};
    candidatos.forEach(c => c.skills.forEach((s) => {
